// Bigger E2E scraper — fetches HN top 500 stories + comments for top 50, dumps EVERYTHING to log
import { writeFileSync, appendFileSync, statSync, readFileSync } from 'node:fs';
...
const RUN_ID = `hn-big-${RUN_START}`;

console.log(`\n========== BIG SCRAPE RUN ${RUN_ID} ==========`);
console.log(`Started: ${RUN_TS}`);
console.log(`Process: pid=${process.pid}, node=${process.version}`);

// Tuned to stay under 15 min — 500 stories + 50 stories w/ comments × 30 = 2000 comments
const TOP_N_STORIES = 500;        // FULL HN top list
const STORIES_WITH_COMMENTS = 50; // top 50 stories get their comments fetched
const COMMENTS_PER_STORY = 30;    // 30 comments each = 1500 comments
...
for (let i = 0; i < storyIds.length; i++) {
  ...
  try {
    ...
    if ((i + 1) % 50 === 0 || i === storyIds.length - 1) {
...
for (let i = 0; i < STORIES_WITH_COMMENTS; i++) {
  ...
  if ((i + 1) % 20 === 0 || i === STORIES_WITH_COMMENTS - 1) {
...
console.log(`\n[5/5] Dumping ENTIRE raw scrape to log (for log-as-output audit) ...`);
console.log(`\n========== RAW_SCRAPE_DUMP_START ==========`);
console.log(`# Total lines: ${stories.length + commentCount}`);
console.log(`# Total bytes: ${rawSize}`);
// Read the raw file and dump line-by-line to stdout with explicit flush
// Using readline-style chunked output to avoid memory issues with large strings
...
for await (const line of rl) {
  lineNum++;
  // Print each JSONL line with a small prefix for parseability
  process.stdout.write(line + '\n');
  if (lineNum % 200 === 0) {
    // Periodically flush and report progress
    await new Promise(r => setImmediate(r));
  }
}
console.log(`# Dumped ${lineNum} lines`);
console.log(`========== RAW_SCRAPE_DUMP_END ==========`);

console.log(`\n========== PROCESSED_SUMMARY_START ==========`);
console.log(processedJson);
console.log(`========== PROCESSED_SUMMARY_END ==========`);

console.log(`\n========== BIG SCRAPE RUN ${RUN_ID} COMPLETED ==========`);
